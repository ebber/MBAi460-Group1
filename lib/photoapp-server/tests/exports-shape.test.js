...
test('repositories is null pending Phase 0.3', () => {
  // CL9 SQL-into-repositories reconciliation lands in Phase 0.3; until then
  // services.photoapp keeps its inline SQL. This test exists to make the
  // pending-state explicit; flip to expect(Object.keys(lib.repositories))
  // when 0.3 lands.
  expect(lib.repositories).toBeNull();
});
