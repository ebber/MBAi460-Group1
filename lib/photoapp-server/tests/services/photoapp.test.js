...
const { getPing, listUsers, listImages, getImageLabels, searchImages, uploadImage, downloadImage, deleteAll } = require('../../src/services/photoapp');
...
describe('getImageLabels()', () => {
  ...
  test('known assetid with labels returns mapped rows ordered by label ASC', async () => {
    // Iter-15: ORDER BY label ASC. See lib/photoapp-server/src/repositories/labels.js
    ...
    expect(labelsSql).toMatch(/SELECT label, confidence FROM labels WHERE assetid = \? ORDER BY label ASC/);
    expect(fakeDb.end).toHaveBeenCalled();
  });
});
