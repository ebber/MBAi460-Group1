...
const {
  ...
  cleanupTempFile,
} = require('../../../src/photoapp-core/middleware/upload');
...
test('upload middleware accepts a JPEG (photo)', async () => {
  ...
  const res = await request(app).post('/test-upload').attach('file', Buffer.from('fake'), {
    filename: 'a.jpg',
...
test('upload middleware rejects files over 50 MB', async () => {
  ...
  app.use((err, req, res, _next) => res.status(400).json({ error: err.code }));

  const big = Buffer.alloc(51 * 1024 * 1024);
  const res = await request(app).post('/test-upload').attach('file', big, {
    filename: 'huge.bin',
...
test('cleanupTempFile is a no-op when the file is missing', () => {
  expect(() => cleanupTempFile(path.join(os.tmpdir(), 'definitely-not-here.bin'))).not.toThrow();
});
