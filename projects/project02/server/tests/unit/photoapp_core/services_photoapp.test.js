...
describe('getImageLabels()', () => {
  ...
  test('known assetid with labels returns mapped rows ordered by label ASC', async () => {
    // Iter-15: ORDER BY label ASC. See projects/project02/server/src/photoapp-core/repositories/labels.js
...
describe('deleteAll()', () => {
  ...
  test('non-empty: DELETE FROM labels BEFORE DELETE FROM assets, S3 DeleteObjects AFTER both', async () => {
    const fakeDb = {
      execute: jest
        .fn()
        .mockResolvedValueOnce([
          [{ bucketkey: 'p_sarkar/uuid-a.jpg' }, { bucketkey: 'p_sarkar/uuid-b.pdf' }],
        ])
        .mockResolvedValueOnce([{}])
        .mockResolvedValueOnce([{}])
        .mockResolvedValueOnce([{}]),
  ...
  test('resets assets AUTO_INCREMENT to 1001 after DELETE FROM assets', async () => {
    ...
    const sqls = fakeDb.execute.mock.calls.map((c) => c[0]);
    expect(sqls).toEqual(
      expect.arrayContaining([expect.stringMatching(/ALTER TABLE assets AUTO_INCREMENT = 1001/)]),
    );
