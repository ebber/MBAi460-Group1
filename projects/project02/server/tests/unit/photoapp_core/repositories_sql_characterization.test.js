// projects/project02/server/tests/unit/photoapp_core/repositories/sql-characterization.test.js
//
...
const usersRepo = require('../../../src/photoapp-core/repositories/users');
const assetsRepo = require('../../../src/photoapp-core/repositories/assets');
const labelsRepo = require('../../../src/photoapp-core/repositories/labels');
...
describe('SQL characterization: assets repository', () => {
  test('findAll — passes empty params array explicitly (preserves Part 03 byte-identical call shape)', async () => {
    ...
    expect(conn.calls).toEqual([
      {
        sql: 'SELECT assetid, userid, localname, bucketkey, kind FROM assets ORDER BY assetid ASC',
        params: [],
      },
    ]);
  });
  ...
  test('findByUserId', async () => {
    ...
    expect(conn.calls).toEqual([
      {
        sql: 'SELECT assetid, userid, localname, bucketkey, kind FROM assets WHERE userid = ? ORDER BY assetid ASC',
        params: [80001],
      },
    ]);
  });
  ...
  test('findById — full-shape projection without kind (downloadImage path)', async () => {
    ...
    expect(conn.calls).toEqual([
      {
        sql: 'SELECT assetid, userid, localname, bucketkey FROM assets WHERE assetid = ?',
        params: [1001],
      },
    ]);
  });
  ...
  test('selectAllBucketkeys — single-arg execute (no params)', async () => {
    ...
    expect(conn.calls).toEqual([{ sql: 'SELECT bucketkey FROM assets', params: undefined }]);
  });

  test('deleteAll — single-arg execute', async () => {
    const conn = makeRecordingConn([{}]);
    await assetsRepo.deleteAll(conn);
    expect(conn.calls).toEqual([{ sql: 'DELETE FROM assets', params: undefined }]);
  });
...
describe('SQL characterization: labels repository', () => {
  test('findByAssetId — projection: label + confidence; ORDER BY label ASC', async () => {
    // Iter-15: ORDER BY label ASC (alphabetical) — assignment template's
    // expected output order. See projects/project02/server/src/photoapp-core/repositories/labels.js
    // for the contract-correction comment.
    const conn = makeRecordingConn([[]]);
    await labelsRepo.findByAssetId(conn, 1001);
    expect(conn.calls).toEqual([
      {
        sql: 'SELECT label, confidence FROM labels WHERE assetid = ? ORDER BY label ASC',
        params: [1001],
      },
    ]);
  });
  ...
  test('insertOne — INSERT IGNORE; columns: assetid, label, confidence; FLOOR(confidence) in VALUES', async () => {
    // Iter-15: FLOOR(?) — Python int() truncation semantics. See
    // projects/project02/server/src/photoapp-core/repositories/labels.js for contract-correction comment.
    const conn = makeRecordingConn([{ insertId: 1, affectedRows: 1 }]);
    await labelsRepo.insertOne(conn, 1001, 'Dog', 99.5);
    expect(conn.calls).toEqual([
      {
        sql: 'INSERT IGNORE INTO labels(assetid, label, confidence) VALUES (?, ?, FLOOR(?))',
  ...
  test('deleteAll — single-arg execute', async () => {
    ...
    expect(conn.calls).toEqual([{ sql: 'DELETE FROM labels', params: undefined }]);
  });
});
