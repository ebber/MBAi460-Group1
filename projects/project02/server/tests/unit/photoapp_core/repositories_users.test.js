// projects/project02/server/tests/unit/photoapp_core/repositories/users.test.js
//
...
describe('countAll', () => {
  test('returns user count from SELECT count(userid)', async () => {
    ...
    expect(fakeConn.execute).toHaveBeenCalledWith('SELECT count(userid) AS num_users FROM users');
  });
