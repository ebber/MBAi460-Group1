// projects/project02/server/tests/unit/photoapp_core/schemas/rows.test.js
//
...
const {
  ...
  searchRowToObject,
  deriveKind,
} = require('../../../src/photoapp-core/schemas/rows');
...
test('searchRowToObject normalizes a (assetid, label, confidence) row', () => {
  expect(searchRowToObject({ assetid: 1001, label: 'Animal', confidence: 99 })).toEqual({
    assetid: 1001,
    label: 'Animal',
    confidence: 99,
  });
