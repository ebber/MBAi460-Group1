...
- [x] **Phase 1.1** — docker-compose + LocalStack ✅ 2026-05-04 — `docker-compose.yml` (mysql:8.4 + localstack:3 + server; client placeholder commented); `infra/migrations/01-schema.sql` (combined schema + seed users mounted to `/docker-entrypoint-initdb.d/`); `tools/bootstrap-localstack.sh` (idempotent bucket + IAM user create via `aws --endpoint-url`); `client/photoapp-config.ini.example` (compose-specific template with LocalStack test credentials); `docker compose config` validates; `make up`/`make down` wired in Makefile. Note: `forcePathStyle: true` may be needed for LocalStack S3 — deferred to first `make up` smoke run (SDK picks up `AWS_ENDPOINT_URL` automatically; `forcePathStyle` is a follow-up CL9 if needed).
- [x] **Phase 1.2** — Terraform module refactor (rds/, s3/, iam/, cloudwatch/ skeleton) ✅ 2026-05-04 — `infra/modules/{rds,s3,iam,cloudwatch}/{main,variables,outputs}.tf` extracted from flat `MBAi460-Group1/infra/terraform/main.tf`; `infra/envs/{dev,prod}/{main,variables,outputs}.tf` + `terraform.tfvars.example`; remote state backend (S3 + DynamoDB) commented as placeholder (lands Part 02); D10 forward-only — no `terraform apply` in Part 01.
- [x] **Phase 1.3** — pino + pino-http structured logging ✅ 2026-05-04 — `server/observability/pino.js` (single logger instance; level honours `LOG_LEVEL`/`NODE_ENV`; base `{service: 'photoapp-web', env}`; pino-pretty transport in non-prod for human-readable colorized output, raw JSON in prod for stdout→CloudWatch); `server/middleware/logging.js` (pino-http with `genReqId: req => req.id`, slim req/res serializers); wired into `app.js` after `request_id`. `server.js` now uses pino for the `web service listening` log + the SIGTERM `draining…` warn line. Smoke-verified: pretty pino startup, per-request log lines, X-Request-Id round-trip from client → log → response. 3 unit tests (pino API + base fields + level) green.
- [x] **Phase 1.4** — request_id middleware ✅ 2026-05-04 — `server/middleware/request_id.js` reads `X-Request-Id` header if present, otherwise generates `uuidv4`; sets `req.id`; echoes the header back on the response. Both branches unit-tested + the integration test confirms the header round-trips through `/healthz`. Mounted FIRST in `app.js` so pino-http (3.3) sees `req.id` populated.
- [x] **Phase 1.5** — error middleware via library factory + AppError hierarchy ✅ 2026-05-04 — `middleware/errors.js` (AppError + 4 subclasses); `middleware/error_config.js` (mount-prefix-aware `statusCodeMap` + `errorShapeFor`); `app.js` wires `middleware.createErrorMiddleware({ statusCodeMap, errorShapeFor, logger })`; 23 unit tests (errors.test.js 13/13 + error_middleware.test.js 10/10).
- [x] **Phase 1.6** — validate middleware (zod-based) ✅ 2026-05-04 — `middleware/validate.js` + `schemas/request_schemas.js` placeholder; `validate({body,params,query})` factory → BadRequestError on ZodError; 5 unit tests.
- [x] **Phase 1.7** — mysql2 pool factory + opossum breakers ✅ 2026-05-04 — `services/pool.js` (getPool memoised singleton + closePool idempotent); `services/breakers.js` (getBucketBreaker + getRekognitionBreaker, opossum wrappers); 10 unit tests (pool 5/5 + breakers 5/5).
- [x] **Phase 1.8** — OpenAPI 3.1 stub + library exports verified ✅ 2026-05-04 — CL9 library change: `successResponse({...extras})` variadic (Part 03 callsites updated); `tests/unit/envelopes_spec_shapes.test.js` (9 tests locking spec shapes); `api/openapi.yaml` covering all 8 spec routes + reusable component schemas; `tests/contract/openapi_internal_consistency.test.js` (swagger-parser.validate 1/1).
...
- [x] **Phase 1.10** — docker-compose orchestration validated ✅ 2026-05-04 — `make up`/`make down` wired; `/readyz` (Phase 4) probes RDS pool + S3; `server.js` calls `closePool()` in SIGTERM handler; `app.js` mounts `/readyz`; `docker compose config` validates; unit tests (60/60) + contract (1/1) + lint clean post-wiring.
- [x] **Phase 1.11** — six-layer test pyramid harness ✅ 2026-05-04 — `jest.config.js` multi-project (unit/integration/contract/smoke/happy/live); smoke skeleton (1/1 /healthz); happy_path skeleton (8 skipped); live skeleton (gated on PHOTOAPP_RUN_LIVE_TESTS=1); Python client harness deferred to workstream 03.
- [x] **Phase 1.12** — Terraform `state mv` cutover ✅ 2026-05-04 — D10 decision: forward-only; no `state mv` in Part 01. Flat `MBAi460-Group1/infra/terraform/` remains the applied dev env. Module tree at `projects/project02/infra/` is the Part 02 deployment target; `terraform validate` deferred until Terraform is installed locally (Terraform not installed in this environment). `terraform.tfvars.example` files guide the cutover workflow.
