...
- [ ] **Step 4** — End-to-end verification:
  - [ ] **Step 4 sub-frame (conditional)** — Lab spin-up if `utils/lab-status` shows down: `utils/lab-up` → wait for RDS ready → `utils/smoke-test-aws --mode live` 10/10. Lab status as of 2026-05-04: DOWN (3/10) → sub-frame is non-optional. If `utils/lab-up` fails → troubleshooting sub-frame; do NOT proceed to push-readiness with lab gates unresolved.
