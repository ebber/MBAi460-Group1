...
- 🔄 **Step 3** — Execute merge sequence (chunked per 2c; tests after each chunk; inline merge-execution notes). **Chunk 1 ✅ 2026-05-04** — integration branch `merge/collab-reconciliation` created from `main`; 6 Foundation commits cherry-picked code-only (e6923d3 → 6347c95 → 78fb7db → 7a5131c → 5581051 → 2e88078, becoming 92cb003 → 85e971b → 8a8a4bd → f7f18ab → 8d2fc38 → fd62ecd on integration branch). Tracker filter clean (no MetaFiles/Approach or legacy_ leakage). Close gate: lib 104/104 (CL9 +5 verified) + Part 03 32+2 skipped (no regression) + project02/server 64+13 skipped (matches Step-2-verification baseline) + lib-symlink-check 7/7. Conflicts on tracker files resolved per documented mechanic (legacy_PlanningOrientationMap.md → take HEAD; client/MetaFiles/refactor-log.md → git rm). **Chunk 2 (skip 685b501) handled implicitly via not cherry-picking; Chunk 3 (port B's routes into A's structure) is the next sub-frame.**
- [ ] **Step 4** — End-to-end verification:
  - 🟡 **Step 4 sub-frame** — **AWS lab spin-up PINNED 2026-05-04 (Erik stepping away mid-Chunk-6).** Discovery: `utils/lab-up` at lab-repo level is Docker-only (delegates to docker-up). AWS-level utility built: `utils/aws-probe` (read-only state diagnostic; commit `711254c`). **Resumption:** Erik runs `utils/aws-probe` from his terminal; recommended-action output drives next step. Lab was stopped (not destroyed) by external force per Erik 2026-05-04 → likely just `aws rds start-db-instance --db-instance-identifier <id>` after probe. Stages B (schema seed via `utils/rebuild-db` + `utils/validate-db`) / C (live regression `PHOTOAPP_RUN_LIVE_TESTS=1` against Part 03 + project02/server) / D (verify via `utils/aws-probe` again for 10/10 + `cd projects/project02/client && pytest` full battery) blocked on Stage A output. **Do NOT proceed to push-readiness with lab gates unresolved.**
  - [ ] Full battery: workspace tests / freshclone-smoke / docker build / package-submission / live regression / smoke-test-aws
  - [ ] **🛑 Step 4 close → Step 5 entry pause** (Mermaid-checkpoint-pattern, added 2026-05-04 per Erik directive). Print summary + test results to chat: all gate outcomes / lab state / file-level diff since main's d2e039c / surprises / memory candidates / tracker stale-state acknowledgment. Hold for Erik greenlight before Step 5 entry. Trackers are durable; verifying on-disk reality before authoring them is verification-as-its-own-work.
