// projects/project01/Part03/server/tests/photoapp_core/repositories/sql-characterization.test.js
//
...
describe("SQL characterization: users repository", () => {
  test("countAll", async () => {
    ...
    expect(conn.calls).toEqual([
      {
        sql: "SELECT count(userid) AS num_users FROM users",
        params: undefined,
      },
    ]);
  });
...
describe("SQL characterization: labels repository", () => {
  test("findByAssetId — projection: label + confidence; ORDER BY label ASC", async () => {
    ...
    // expected output order. See projects/project01/Part03/server/src/photoapp-core/repositories/labels.js
  ...
  test("insertOne — INSERT IGNORE; columns: assetid, label, confidence; FLOOR(confidence) in VALUES", async () => {
    ...
    // projects/project01/Part03/server/src/photoapp-core/repositories/labels.js for contract-correction comment.
