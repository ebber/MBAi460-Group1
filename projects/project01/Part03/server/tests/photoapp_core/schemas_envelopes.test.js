// projects/project01/Part03/server/tests/photoapp_core/schemas/envelopes.test.js
//
...
const {
  successResponse,
  errorResponse,
} = require("../../src/photoapp-core/schemas/envelopes");
...
test("successResponse({M, N}) produces ping envelope", () => {
  expect(successResponse({ M: 5, N: 10 })).toEqual({
    message: "success",
    M: 5,
    N: 10,
  });
...
test("successResponse({assetid}) produces upload envelope", () => {
  expect(successResponse({ assetid: 42 })).toEqual({
    message: "success",
    assetid: 42,
  });
});
