// projects/project01/Part03/server/tests/photoapp_core/repositories/assets.test.js
//
...
describe("insert", () => {
  test("inserts asset and returns OkPacket with insertId", async () => {
    const fakeConn = {
      execute: jest
        .fn()
        .mockResolvedValue([{ insertId: 1001, affectedRows: 1 }]),
    };
  ...
  test("returns array of bucketkey strings", async () => {
    const fakeConn = {
      execute: jest
        .fn()
        .mockResolvedValue([
          [{ bucketkey: "p/u-a.jpg" }, { bucketkey: "p/u-b.pdf" }],
        ]),
    };
