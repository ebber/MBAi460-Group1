jest.mock("../../src/photoapp-core/services/aws");
jest.mock("../../src/photoapp-core/middleware/upload");
jest.mock("fs", () => ({
  ...jest.requireActual("fs"),
  readFileSync: jest.fn(() => Buffer.from("fake-bytes")),
}));

const aws = require("../../src/photoapp-core/services/aws");
const upload = require("../../src/photoapp-core/middleware/upload");
const {
  getPing,
  listUsers,
  listImages,
  ...
  downloadImage,
  deleteAll,
} = require("../../src/photoapp-core/services/photoapp");
      ...
      execute: jest.fn().mockResolvedValue([
        [
          ...
          },
          {
            assetid: 1002,
            userid: 80001,
            localname: "lecture.pdf",
            bucketkey: "p_sarkar/uuid-lecture.pdf",
            kind: "document",
          },
        ],
      ]),
...
describe("getImageLabels()", () => {
  ...
  test("known assetid with labels returns mapped rows ordered by label ASC", async () => {
    // Iter-15: ORDER BY label ASC. See projects/project01/Part03/server/src/photoapp-core/repositories/labels.js
    // for the contract-correction rationale.
    const fakeDb = {
      execute: jest
        .fn()
        .mockResolvedValueOnce([[{ assetid: 1001 }]])
...
describe("uploadImage()", () => {
  ...
  test("rejects unknown userid", async () => {
    ...
    await expect(uploadImage(99999, fakeFile)).rejects.toThrow(
      "no such userid",
    );
  ...
  test("round-trips through S3, Rekognition, and INSERT (photo path)", async () => {
    ...
    const assetsCall = fakeDb.execute.mock.calls.find((c) =>
      /INSERT INTO assets/.test(c[0]),
    );
...
describe("downloadImage()", () => {
  ...
  test("content-type falls back to extension map when S3 has no ContentType", async () => {
    ...
    aws.getBucket.mockReturnValue({
      send: jest.fn().mockResolvedValue(s3Result),
    });
    aws.getBucketName.mockReturnValue("test-bucket");
          ...
          [
            {
              ...
              userid: 80001,
              localname: "data.bin",
              bucketkey: "p_sarkar/uuid-data.bin",
            },
          ],
        ]),
      end: jest.fn().mockResolvedValue(),
...
describe("deleteAll()", () => {
  ...
  test("resets assets AUTO_INCREMENT to 1001 after DELETE FROM assets", async () => {
    ...
    const alterIdx = sqls.findIndex((s) =>
      /ALTER TABLE assets AUTO_INCREMENT = 1001/.test(s),
    );
