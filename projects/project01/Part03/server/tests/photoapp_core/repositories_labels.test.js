// projects/project01/Part03/server/tests/photoapp_core/repositories/labels.test.js
//
...
describe("findByAssetId", () => {
  // Iter-15 contract correction: assignment template orders labels alphabetically
  // (ORDER BY label ASC), not by confidence. Confirmed by Project 02 autograder
  // Test 3 / test_27/28/32 — expected lists are alphabetical.
  test("returns labels ordered by label ASC for the assetid", async () => {
    const fakeConn = {
...
describe("insertOne", () => {
  // Iter-15 contract correction: assignment template uses Python int() to truncate
  // Rekognition's float confidence (e.g., 99.7 → 99). SQL FLOOR(?) reproduces this.
  // SQL ROUND() would round-to-nearest (99.5+ → 100), which mismatches by ±1.
  // Confirmed by Project 02 autograder Test 3 / test_27/28/32/35/37/38.
  test("issues INSERT IGNORE with assetid + name + FLOOR(confidence)", async () => {
    const fakeConn = { execute: jest.fn().mockResolvedValue([{}]) };
    await labelsRepo.insertOne(fakeConn, 1001, "Animal", 99.5);
    expect(fakeConn.execute).toHaveBeenCalledWith(
      "INSERT IGNORE INTO labels(assetid, label, confidence) VALUES (?, ?, FLOOR(?))",
      [1001, "Animal", 99.5],
    );
  });
});
