// projects/project01/Part03/server/src/photoapp-core/services/photoapp.js
//
...
async function uploadImage(userid, multerFile) {
  ...
  try {
    ...
    const insertResult = await assetsRepo.insert(dbConn, {
      userid,
      localname,
      bucketkey,
      kind,
    ...
    if (kind === "photo") {
      const rekogResult = await aws.getRekognition().send(
...
async function downloadImage(assetid) {
  ...
  try {
    ...
    const { userid, localname, bucketkey } = asset;

    const s3Result = await aws.getBucket().send(
