// projects/project01/Part03/server/src/photoapp-core/repositories/labels.js
//
...
async function findByAssetId(conn, assetid) {
  const [rows] = await conn.execute(
    // ORDER BY label ASC — matches assignment template's expected output order
    // (alphabetical), confirmed by Project 02 autograder Test 3 / iter-15.
...
async function insertOne(conn, assetid, name, confidence) {
  await conn.execute(
    // FLOOR(?) — matches assignment template's Python `int(confidence)` semantics
    // (truncation, not round-to-nearest). Confirmed by Project 02 autograder Test 3
    // off-by-one failures across test_27/28/32/35/37/38 — Rekognition returns
    // floats like 99.7; Python int() → 99; SQL ROUND() → 100. iter-15 fix.
