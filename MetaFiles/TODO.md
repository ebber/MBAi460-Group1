...
- [ ] **[Test/DB] `validate-db` `assets`-empty assertion drift** — `MBAi460-Group1/utils/validate-db` asserts the `assets` table is empty (per `_validate_db.py`'s "Seed data: assets table (expect empty)" check). Observed during 2026-05-01 spin-up Environmental Spin: **14 rows present**. Test-vs-reality drift, not a credentials/RDS-health issue — other 26/27 checks PASS, including all three RDS identities (`admin` + `photoapp-read-only` + `photoapp-read-write`). Two paths forward: (a) update the test's expected baseline to reflect the current dev/seed state, or (b) re-baseline by clearing the `assets` table if rows aren't part of the expected seed state. Inspect the 14 rows first (`SELECT assetid, userid, bucketkey FROM assets;`) to inform the call. (Spin-Up Ritual EOR, 2026-05-01)

- [ ] **[CI/Future-State] Phase E — Playwright E2E GitHub Actions workflow** — DEFERRED 2026-05-04 per Erik (VCS out of scope for current arc). Scope when activated: `.github/workflows/playwright.yml` calling `utils/e2e-smoke --no-destructive` on PRs touching `projects/project01/Part03/**`, `lib/photoapp-server/**`, `.github/workflows/playwright.yml`, or `utils/e2e-smoke`. Prerequisites: (1) provision a scoped IAM identity (`e2e-ci` user with `s3:PutObject/GetObject/DeleteObject` on `photoapp-erik-mbai460/*`, RDS connect via existing `photoapp-read-write`, `rekognition:DetectLabels` only — NOT `Claude-Conjurer` PowerUserAccess); (2) GHA secrets (`E2E_AWS_ACCESS_KEY_ID`, `E2E_AWS_SECRET_ACCESS_KEY`, plus any additional config substitutions); (3) config-materialization template at `.github/ci/photoapp-config.ini.tmpl` (the live `photoapp-config.ini` is gitignored). Cost per run: ~$0.001 Rekognition + ~3-4 GHA minutes; bounded by path filter. Suggested deployment shape: ship with `workflow_dispatch` trigger only initially, verify manually, then flip on `pull_request` triggers in a follow-up — avoids first-PR-broken-secrets chicken-and-egg. Note: branch protection enforcement is itself queued (per CONTRIBUTING.md "pending Phase 0.6.3 landing"); until that lands, workflow is informational, not gating. (Playwright E2E workstream Phase E deferral, 2026-05-04)

- [ ] **[API/Library Polish] Implement `DELETE /api/images/:id` (single-asset delete + tests)** — surfaced 2026-05-04 during the Playwright E2E workstream Phase B↔B-sidecar boundary review. Current state: only `DELETE /api/images` (deleteAll) exists; no single-asset delete in API (`server/routes/photoapp_routes.js:118`), service (`lib/photoapp-server/src/services/photoapp.js`), client (`frontend/src/api/photoappApi.ts:54`), or UI (`LibraryPage.tsx:115` only has "Delete all"). **Already named in `Future-State-roadmap.md` Tier 1 Library Polish workstream** as a planned new endpoint. Scope: backend route + service method + repository methods (delete labels first, then asset row, then S3 object) + frontend client `deleteImage(id)` + Jest+supertest tests + UI button on AssetDetail or Library card + lib version bump per CONTRIBUTING. Pulled into TODO queue per Erik 2026-05-04: the gap blocked Playwright's per-test specific-asset cleanup, so the current Playwright suite uses the destructive `deleteAll` opt-in spec at session end as a coarser cleanup mechanism instead — implementing this endpoint would let Playwright assert specific-asset deletion (Erik's stated preference) without scope-creeping into Library Polish during the E2E workstream itself. (Playwright E2E workstream B-sidecar inspect-before-writing finding, 2026-05-04)
