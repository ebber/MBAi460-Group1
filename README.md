...
- `lib/*` — shared/future shared libraries (currently `lib/photoapp-server`, retained for Project 01)
- `projects/project01/Part03` — Part 03 PhotoApp web service (currently a consumer of `lib/photoapp-server`)
...
After install, the lib remains reachable for current consumers via `require('@mbai460/photoapp-server')`. Project 02 no longer uses that runtime path; it owns a copied local core for the split MVP. Lockfiles are root-only — there is exactly one `package-lock.json`.
